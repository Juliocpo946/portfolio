...
import { projectsData, skills, certifications } from '../data/content';
          ...
          <div className="grid md:grid-cols-2 gap-12 mt-12">
             <RevealText delay={0.5}>
               <p className="text-lg md:text-xl font-light leading-relaxed text-neutral-500 dark:text-neutral-400 max-w-md">
                 {t.hero.desc} <span className="text-black dark:text-white font-medium">GIINTAPE INNOVAHUE</span>.
               </p>
             </RevealText>
        ...
        <section ref={gridRef} className="px-6 md:px-12 min-h-screen overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             <motion.div style={{ y: yCol1 }} className="flex flex-col gap-6">
               <ProjectCard project={projectsData[0]} label={t.projects.hover} onClick={handleProjectClick} />
               <ProjectCard project={projectsData[1]} label={t.projects.hover} onClick={handleProjectClick} />
             </motion.div>
             <motion.div style={{ y: yCol2 }} className="flex flex-col gap-6 pt-0 md:pt-24">
               <ProjectCard project={projectsData[2]} label={t.projects.hover} onClick={handleProjectClick} />
             ...
             <motion.div style={{ y: yCol3 }} className="flex flex-col gap-6 pt-0 lg:pt-12 hidden lg:flex">
               <ProjectCard project={projectsData[3]} label={t.projects.hover} onClick={handleProjectClick} />
               <ProjectCard project={projectsData[4]} label={t.projects.hover} onClick={handleProjectClick} />
             </motion.div>
          </div>
              ...
              <div className="flex flex-col gap-6 items-start xl:items-end">
                 <div className="flex gap-4">
                    {['LinkedIn', 'GitHub', 'Instagram'].map((social) => (
                      <a key={social} href="#" className="border border-neutral-400 dark:border-neutral-600 px-6 py-3 text-xs uppercase tracking-widest hover:bg-current hover:text-white dark:hover:text-black transition-colors">{social}</a>
                    ))}
                 </div>
